	...
	function mbw_get_category_template($type, $data=""){
		...
		if(strpos($type, "SELECT")===0 && $data!=""){
			...
			if(!$hierarchical){
				...
				$template_category	.= '<input type="hidden" name="'.mbw_set_form_name("category2").'" value="' . $category2 . '" />';
				$template_category	.= '<input type="hidden" name="'.mbw_set_form_name("category3").'" value="' . $category3 . '" />';
			}else{
				if(mbw_get_board_item("fn_category1")!="") $category1				= mbw_htmlspecialchars_decode(mbw_get_board_item("fn_category1"));
		...
		// 카테고리를 탭메뉴로 보여주기
			...
			$template_category	.= '<input type="hidden" name="'.mbw_set_form_name("category1").'" value="' . (htmlspecialchars($category1)) . '" />';
			if($category1=="" && $category2!="") mbw_set_param("category2","");
			$template_category	.= '<input type="hidden" name="'.mbw_set_form_name("category2").'" value="' . (htmlspecialchars($category2)) . '" />';
			if($category2=="" && $category3!="") mbw_set_param("category3","");
			$template_category	.= '<input type="hidden" name="'.mbw_set_form_name("category3").'" value="' . (htmlspecialchars($category3)) . '" />';
		}else{
			$template_category	.= '<input type="hidden" name="'.mbw_set_form_name("category1").'" value="' . (htmlspecialchars($category1)) . '" />';
			if($category1=="" && $category2!="") mbw_set_param("category2","");
			$template_category	.= '<input type="hidden" name="'.mbw_set_form_name("category2").'" value="' . (htmlspecialchars($category2)) . '" />';
