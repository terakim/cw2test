			...
			if(mbw_get_vars("pagination_type")=="large" || mbw_get_vars("pagination_type")=="middle2" || mbw_get_vars("pagination_type")=="middle3"){
				...
				if($block_first == 1) $page_link["prev"] = "";
					...
					if($link_type=="href"){
						...
						$page_link["prev"] = "sendListTemplateData({'board_name':'".$board_name."','page':'".($prev_board_page)."','page_type':'ajax'})";
					}
				}
				$link_page	= "";
				$first_class	= "";
				...
				for($i = $block_first; $i <= $block_last; $i++){
					...
					if($i == $board_page){
						...
						if($link_type=="href"){
							$link_page .= '<td class="btn-page"><a class="btn-page-num'.$first_class.'" '.$link_type.'="'.mbw_get_url(array("board_pid"=>"","board_page"=>$i)).'" title="Page '.$i.'">'.$i.'</a></td>';
						}else if($link_type=="onclick"){
							$link_page .= '<td class="btn-page"><a class="btn-page-num'.$first_class.'" '.$link_type."=\"sendListTemplateData({'board_name':'".$board_name."','page':'".$i."','page_type':'ajax'})\" title='Page ".$i."'>".$i.'</a></td>';
						}
					}
				}
				...
				if($block_last == $total_page) $page_link["next"] = "";
					...
					if($link_type=="href"){
						...
						$page_link["next"] = "sendListTemplateData({'board_name':'".$board_name."','page':'".$next_board_page."','page_type':'ajax'})";
					}				
				}
				...
				foreach($page_link as $key =>$value){
					if(empty($value)) $page_class[$key]		= ' class="page_arrow mb-disabled'.$page_class[$key].'"';
					else $page_class[$key]		= ' class="page_arrow'.$page_class[$key].'"';
				}
				...
				foreach($page_link as $key =>$value){
					if(empty($page_link[$key])) $page_class[$key]		= ' class="mb-disabled'.$page_class[$key].'"';
				}
