...
if(!function_exists('mbw_get_btn_template')){
	function mbw_get_btn_template($data, $args=NULL){
		...
		if(!empty($data['img'])){
			...
			if(!empty($data['img_style'])) $main_tag		= $main_tag.' style="'.$data['img_style'].'"';
			if(!empty($data['img_class'])) $main_tag		= $main_tag.' class="'.$data['img_class'].'"';
			$main_tag		= $main_tag.' />';		
			$btn_type		= "a";
		}
		...
		if(!empty($data['href'])){
			if($btn_type=="a"){
				...
				if(!empty($data['target'])){
					$template_btn	.= ' target="'.$data['target'].'"';
				}
			}else if($btn_type=="button"){
