...
if(!function_exists('mbw_get_tag_template')){
	function mbw_get_tag_template($data){
		...
		else $data["tag_id"]		= 'id="'.$data["tag_id"].'"';

		$template_html		= "";
		if($data["tag_name"]=="table"){
			$template_start	= '<table  cellspacing="0" cellpadding="0" border="0" '.$data["tag_id"].' '.$data["ext"].__STYLE($data["style"]).'><caption>'.$data["name"].'</caption>';
				...
				if($cols_count>1){
					for($i=0;$i<$cols_count;$i++){
						if($cols_width[$i]!="" && $cols_width[$i]!="*")
							$template_start	.= '<col class="mb-col-item'.($i+1).'" style="width:'.$cols_width[$i].'"/>';
						else
							$template_start	.= '<col class="mb-col-item'.($i+1).'" />';
					}
				}
				$template_start	.= '</colgroup>';
			}
			...
			if(!empty($template_html))  $template_start	= '<'.$data["tag_name"].$data["td_class"].__STYLE($data["td_style"]).' '.$data["tag_id"].'>';
			else $template_start	= '<'.$data["tag_name"].$data["ext"].__STYLE($data["style"]).' '.$data["tag_id"].'>';			
