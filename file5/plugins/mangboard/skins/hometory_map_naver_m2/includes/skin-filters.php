...
if(!function_exists('mbw_filter_template_item')){
	function mbw_filter_template_item($item){
		...
		if($item["field"]=="fn_title" && mbw_get_param("search_field")=="fn_title" && mbw_get_param("search_text")!=""){
		...
		if($item["field"]=="fn_user_name"){	//USER NAME  FILTER
			...
			if(mbw_get_param("mode")=="view" || mbw_get_param("mode")=="comment"){
				if(mbw_get_param("mode")=="comment"){
					$ip_address		= mbw_get_comment_item("fn_ip");
				}else{
					$ip_address		= mbw_get_board_item("fn_ip");
				}
				$ip_address		= " **".substr($ip_address,2,-2)."**";
			}

			if(!empty($data["user_pid"])){
				$item["value"]		= mbw_get_btn_template(array("name"=>$item["value"],"onclick"=>"getUserMenu('".$data["user_pid"]."','".$data[$mb_fields["board"]["fn_pid"]]."')","title"=>$item["value"].$ip_address,"class"=>"btn-user-info","type"=>"a"));
			}else{
				$item["value"]		= mbw_get_btn_template(array("name"=>$item["value"],"title"=>$item["value"].$ip_address,"class"=>"btn-user-info","type"=>"span"));
			}
			if(mbw_get_param("mode")=="comment" && mbw_get_comment_item("fn_is_secret")=="1"){
				$item["value"]		= '<img class="list-i-secret" alt="secret" src="'.MBW_SKIN_URL.'images/icon_secret.gif" /> '.$item["value"];
			}
			...
			if(mbw_get_option("show_user_picture")){ 
				if(!empty($mb_fields["users"]["fn_user_picture"]) && !empty($data[$mb_fields["users"]["fn_user_picture"]])){
					...
					if(!empty($img_path)){
						if(mbw_get_param("mode")=="comment" ){		//comment 일때 섬네일 표시
							$add_icon1			= '<div><img class="" style="border-radius:100%;vertical-align:middle;width:50px;height:50px;" src="'.$img_path.'" /></div>';
							...
							$item["value"]				= "<div style='display:inline-block;text-align:center;width:70px;'>".$add_icon1.$item["value"]."</div>";
						}else if(mbw_get_param("mode")!="list" || mbw_get_param("list_type")!="gallery"){		//갤러리 리스트가 아닐경우에만 회원 썸네일 표시
							$add_icon1			= '<img class="radius-3" style="vertical-align:middle;max-width:20px;max-height:20px;" src="'.$img_path.'" /> ';
							if(!empty($data["user_pid"])) $add_icon1	= '<a href="javascript:;" onclick="getUserMenu(\''.$data["user_pid"].'\',\''.$data[$mb_fields["board"]["fn_pid"]].'\');return false;" class="btn-user-picture" role="button">'.$add_icon1.'</a>';
					...
					if(mbw_get_param("mode")=="comment" ){
						$add_icon1			= '<div><img class="" style="border-radius:100%;vertical-align:middle;width:50px;height:50px;" src="'.MBW_SKIN_URL.'images/icon_user_default.png" /></div>';
						if(!empty($data["user_pid"])) $add_icon1	= '<a href="javascript:;" onclick="getUserMenu(\''.$data["user_pid"].'\',\''.$data[$mb_fields["board"]["fn_pid"]].'\');return false;" class="btn-user-picture" role="button">'.$add_icon1.'</a>';
						$item["value"]				= "<div style='display:inline-block;text-align:center;width:70px;'>".$add_icon1.$item["value"]."</div>";
					}
				}
