	...
	<?php do_action('mbw_board_skin_search'); ?>
		<div class="list-head">
			...
			<div class="list-search">
				<select id="search_field" name="search_field" class="search-field margin-right-5" title="Search Field"><?php echo $list_data["search"];?></select><input type="text" id="search_text" class="search-text" name="search_text" accesskey="s" title="Search Text" value="<?php echo mbw_htmlspecialchars(mbw_get_param("search_text"));?>" onkeypress="checkEnterKey(sendSearchCalendarData);"/><input style="display:none !important;" type="text"/><?php echo mbw_get_btn_template(array("name"=>"Search","onclick"=>"sendSearchCalendarData()","class"=>"btn btn-default btn-search margin-left-5")); ?>
			</div>
			<?php } ?>
			<div class="clear"></div>
		</div>
