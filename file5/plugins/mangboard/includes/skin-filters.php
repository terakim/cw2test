	...
	function mbw_filter_board_item($item,$data=null){
		...
		if($item["field"]=="fn_user_name"){	//USER NAME  FILTER
			...
			if(!empty($data["user_pid"]) || (mbw_is_admin() && mbw_get_board_name()=="commerce_order")){
				$item["value"]		= mbw_get_btn_template(array("name"=>$item["value"],"onclick"=>"getUserMenu('".$data["user_pid"]."','".$data[$mb_fields["board"]["fn_pid"]]."','".$data[$mb_fields["board"]["fn_user_name"]]."')","title"=>$item["value"].$ip_address,"class"=>"btn-user-info","type"=>"a"));
			}else if(mbw_get_param("mode")=="view" && (mbw_get_board_name()=="users" || mbw_get_board_name()=="commerce_users") && !empty($data["pid"])){
				$item["value"]		= mbw_get_btn_template(array("name"=>$item["value"],"onclick"=>"getUserMenu('".$data["pid"]."','".$data[$mb_fields["board"]["fn_pid"]]."')","title"=>$item["value"].$ip_address,"class"=>"btn-user-info","type"=>"a"));
			}else{
