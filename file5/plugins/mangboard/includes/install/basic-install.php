	...
	function mbw_install_insert_query($insert_prefix,$options,$name="",$field=""){
		...
		if(!empty($name)){
			...
			foreach($rows as $key=>$value){
				$row_check			= 0;
				if(!empty($field))
					$row_check		= intval($wpdb->get_var("SELECT count(*) from ".$name." where ".$field."='".$key."'"));
				if($row_check==0){
...
if(!function_exists('mbw_basic_install')){
	function mbw_basic_install(){
		...
		$options_rows['prevent_content_copy']		= "('setup', 'board', 'W_COPY_PREVENTION', 'prevent_content_copy', '0', '1,0', 'W_ON_OFF', '', '', '', '', 'radio', '<br>MSG_COPY_MOUSE_PREVENT',1)";
		...
		$options_rows['show_user_level']				= "('setup', 'user', 'W_USER_LEVEL_DISPLAY', 'show_user_level', '1', '1,0', 'W_ON_OFF', '', '', '', '', 'radio', '<br>MSG_USER_NAME_LEVEL_SET',1)";
		$options_rows['show_user_picture']			= "('setup', 'user', 'W_USER_THUMBNAILS', 'show_user_picture', '1', '1,0', 'W_ON_OFF', '', '', '', '', 'radio', '<br>MSG_USER_NAME_PHOTO_SET',1)";
		$options_rows['show_name_popup']			= "('setup', 'user', 'W_SHOW_USER_POP', 'show_name_popup', '1', '1,0', 'W_ON_OFF', '', '', '', '', 'radio', '<br>MSG_USER_NAME_POPUP_SET',1)";
		...
		$options_rows['ssl_mode']						= "('setup', 'board', 'W_SSL_CERTIFICATE', 'ssl_mode', '0', '1,0', 'W_ON_OFF', '', '', '', '', 'radio', '<br>MSG_ADDRESS_CERTIFICATE',1)";
		...
		$options_rows['use_seo']						= "('setup', 'board', 'W_SEO', 'use_seo', '1', '1,0', 'W_ON_OFF', '', '', '', '', 'radio', '',1)";
		...
		$options_rows['login_log']						= "('setup', 'log', 'W_LOGIN_LOG', 'login_log', '1', '1,0', 'W_ON_OFF', '', '', '', '', 'radio', '<br>MSG_LOG_SAVE_SHOW',1)";
		$options_rows['point_log']						= "('setup', 'log', 'W_POINT_LOG', 'point_log', '1', '1,0', 'W_ON_OFF', '', '', '', '', 'radio', '<br>MSG_POINT_LOG_SAVE_SHOW',1)";
		$options_rows['error_log']						= "('setup', 'log', 'W_ERROR_LOG', 'error_log', '0', '1,0', 'W_ON_OFF', '', '', '', '', 'radio', '<br>MSG_ERROR_LOG_SHOW',1)";
		$options_rows['referer_log']					= "('setup', 'board', 'W_MENU_REFERER', 'referer_log', '1', '1,0', 'W_ON_OFF', '', '', '', '', 'radio', '',1)";
