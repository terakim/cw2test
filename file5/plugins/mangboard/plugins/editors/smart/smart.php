...
if(!function_exists('mbw_editor_smart_template')){
	function mbw_editor_smart_template($action, $data){
		...
		/*if(mbw_get_vars("device_type")=="mobile" || mbw_get_vars("device_type")=="tablet"){		
			...
			if(empty($data["value"])){
				$data["value"]	= "<p style=\"line-height:1.8;\"><span style=\"font-size:13px;font-family:'나눔고딕','Nanum Gothic',sans-serif;\"><br></span></p>";
			}
			...
			$item_html		.= '<script type="text/javascript">if(typeof(oEditors)==="undefined"){var oEditors = [];};jQuery(document).ready(function(){nhn.husky.EZCreator.createInIFrame({oAppRef: oEditors,elPlaceHolder: "'.$editor_id.'",sSkinURI:"'.$editor_skin.'",fCreator:"createSEditor2",htParams:{bUseToolbar:true,bSkipXssFilter : true,I18N_LOCALE:"'.$editor_locale.'",bUsePhotoUpload:'.$use_photo_upload.',bUseVerticalResizer:true,bUseModeChanger:true,aAdditionalFontList:[["Noto Sans","Noto Sans KR"]]},fOnAppLoad : function(){/*oEditors.getById["'.$editor_id.'"].setDefaultFont("나눔고딕", 9);*/}});   });</script>';	
			echo $item_html;
		}
	}
}
