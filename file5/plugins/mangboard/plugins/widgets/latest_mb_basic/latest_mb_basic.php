...
if(!function_exists('mbw_get_latest_mb_basic')){
	function mbw_get_latest_mb_basic($mode,$data){		
		...
		//필요한 게시판 필드 이름 가져오기
		$select_field			= array("fn_pid","fn_title","fn_category1","fn_category2","fn_category3","fn_image_path","fn_reg_date","fn_comment_count");
		...
		if(has_filter('mf_widget_latest_items')) $latest_list			= apply_filters("mf_widget_latest_items", $latest_list, $data, $where_query, $latest_permalink, basename(dirname(__FILE__)));
		...
		if(!empty($latest_list)){
			foreach($latest_list as $latest_item){
				...
				$latest_html	.= '<tr><td class="mb-latest-item-title">';				
					$latest_html	.= '<a href="'.$latest_permalink.$latest_item[$board_field["fn_pid"]].'" title="'.esc_attr($latest_item[$board_field["fn_title"]]).'">';
						$category1		= $latest_item[$board_field["fn_category1"]];
						if($use_category!="false" && !empty($category1)) $latest_html	.= '<span class="mb-latest-item-category"'.$add_text_style2.'>['.$category1.']</span> ';
						$latest_html	.= '<span class="mb-latest-item-title-text"'.$add_text_style1.'>'.mbw_htmlspecialchars($post_title).'</span>';

						//댓글 개수 표시하기
						$comment_count		= intval($latest_item[$board_field["fn_comment_count"]]);
						if($comment_count>0){
							$latest_html	.= '<span class="mb-latest-item-comment-count"'.$add_comment_style1.'> [<span class="mb-latest-item-comment-count-num"'.$add_comment_style1.'>'.$comment_count.'</span>]</span>';
						}
