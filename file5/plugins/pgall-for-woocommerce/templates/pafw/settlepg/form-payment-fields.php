...
<script>
    jQuery( document ).ready( function ( $ ) {
        ...
        $( 'input.change-card', $wrapper ).on( 'click', function () {
            ...
            $( 'input[name=settlepg_issue_bill_key]', $wrapper ).val( 'yes' );
        } );
    } );
</script>
...
<div class="settlepg-payment-fields">
	...
	<?php endif; ?>
    <input type="hidden" name="settlepg_issue_bill_key" value="<?php echo empty( $bill_key ) || is_account_page() ? 'yes' : 'no'; ?>">
	...
	<?php if ( ! is_account_page() ) : ?>
		<?php if ( 'yes' == pafw_get( $gateway->settings, 'enable_quota', 'no' ) ) : ?>
            <div class="pafw-card-info">
                <div class="fields-wrap">
                    <select name="pafw_settlepg_card_quota">
		...
		<?php else: ?>
            <input type="hidden" name="pafw_settlepg_card_quota" value="00">
		<?php endif; ?>
	<?php endif; ?>
</div>
