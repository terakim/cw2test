...
if ( ! is_account_page() && is_user_logged_in() && 'user' == pafw_get( $gateway->settings, 'management_batch_key', 'subscription' ) ) {
	...
	if ( ! empty( $bill_key ) ) {
		$payment_method_type = get_user_meta( get_current_user_id(), $gateway->get_subscription_meta_key( 'payment_method_type' ), true );
...
<script>
    jQuery( document ).ready( function ( $ ) {
        var $wrapper = $( 'div.kakaopay-payment-fields' );
        ...
        $( 'input.change-card', $wrapper ).on( 'click', function () {
            ...
            $( 'input[name=kakaopay_issue_bill_key]', $wrapper ).val( 'yes' );
        } );
    } );
</script>
...
<div class="kakaopay-payment-fields">
	<?php if ( ! empty( $bill_key ) ) : ?>
        <div class="billing_info" style="margin-bottom: 10px;">
			...
			<?php else: ?>
                <span style="margin-right: 20px; font-weight: bold;"><?php _e( '카카오페이 - MONEY', 'pgall-for-woocommerce' ); ?></span>
			<?php endif; ?>
    ...
    <input type="hidden" name="kakaopay_issue_bill_key" value="<?php echo empty( $bill_key ) ? 'yes' : 'no'; ?>">
</div>
