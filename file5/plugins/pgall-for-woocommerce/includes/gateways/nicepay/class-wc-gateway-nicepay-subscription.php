	...
	if ( ! class_exists( 'WC_Gateway_Nicepay_Subscription' ) ) {
		...
		class WC_Gateway_Nicepay_Subscription extends WC_Gateway_Nicepay {
			...
			public function key_in_payment_form() {
				ob_start();
				wc_get_template( 'pafw/nicepay/form-key-in.php', array( 'gateway' => $this ), '', PAFW()->template_path() );
				ob_end_flush();
			}
			...
			function key_in_payment() {
				try {
					...
					if ( $order ) {
						$bill_key = PAFW_Gateway::issue_bill_key( $order, $this, false );

						PAFW_Gateway::request_subscription_payment( $order, $this, array( 'bill_key' => $bill_key, 'is_renewal' => true ) );
