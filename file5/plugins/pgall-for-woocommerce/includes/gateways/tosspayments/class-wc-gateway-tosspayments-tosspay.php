...
if ( class_exists( 'WC_Payment_Gateway' ) ) {
	...
	if ( ! class_exists( 'WC_Gateway_TossPayments_Tosspay' ) ) {

		class WC_Gateway_TossPayments_Tosspay extends WC_Gateway_TossPayments {

			public function __construct() {
				$this->id = 'tosspayments_tosspay';

				parent::__construct();

				if ( empty( $this->settings['title'] ) ) {
					$this->title       = __( '토스페이', 'pgall-for-woocommerce' );
