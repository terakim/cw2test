...
if ( ! class_exists( 'MSSMS_Settings_SMS_Send' ) ) :
	...
	class MSSMS_Settings_SMS_Send {
		...
		static function init() {
			...
			add_filter( 'msshelper_get_mssms_sms_admin_renewal_options', array( __CLASS__, 'get_send_options' ) );
			add_filter( 'msshelper_get_mssms_sms_user_options', array( __CLASS__, 'get_send_options' ) );
			add_filter( 'msshelper_get_mssms_sms_user_renewal_options', array( __CLASS__, 'get_send_options' ) );
		}
		...
		static function get_default_settings( $target, $order_status ) {
			if ( is_null( self::$default_messages ) ) {
				self::$default_messages['admin'] = apply_filters( 'mssms_default_messages_for_admin', array(
					...
					'pafw-partial-refund' => '[{쇼핑몰명}] {고객명}님 주문번호 #{주문번호} - {부분환불금액} 부분환불 처리가 완료되었습니다.',
				) );
		...
		static function get_admin_setting() {
			...
			return array(
				...
				'elements' => array(
					array(
						'type'     => 'Section',
						'title'    => __( '자동 발송 설정 (일반주문 또는 정기결제 신규주문)', 'mshop-sms-s2' ),
						'elements' => array(
							array(
		...
		static function get_admin_renewal_setting() {
			...
			return array(
				...
				'elements' => array(
					array(
						'type'     => 'Section',
						'title'    => __( '자동 발송 설정 (정기결제 갱신주문)', 'mshop-sms-s2' ),
						'elements' => array(
							array(
								"id"        => "mssms_sms_admin_renewal_options",
								"className" => "sms_admin_option_table",
								"type"      => "SortableTable",
								"sortable"  => true,
								"repeater"  => true,
			...
			return array(
				...
				'elements' => array(
					array(
						...
						'elements' => array(
							array(
								"id"        => "mssms_sms_user_renewal_options",
								"className" => "sms_user_option_table",
								"type"      => "SortableTable",
								"sortable"  => true,
								"repeater"  => true,
