...
if ( ! class_exists( 'Wt_Promotion_Banner' ) )  {
	class Wt_Promotion_Banner {
		...
		private static $webtoffee_url = 'https://www.webtoffee.com/';
				...
				<div class="wt_promotion_banner notice-success notice is-dismissible">
					<div class="wt_promotion_banner_inner">
						...
						<div class="wt_promotion_banner_right">
							...
							<p style = "font-size:15px" ><?php esc_html_e( 'Black Friday & Cyber Monday sale is now live! Get any WebToffee plugins at an exclusive 30% discount.' ); ?></p>
		...
		public function add_banner_scripts() {
			...
			if( $this->is_show_banner() ) { // Show the banner
				...
				?>
		        <style type="text/css">
		            .wt_promotion_banner{ background:#fff url('<?php echo esc_url(WT_O_IEW_PLUGIN_URL );?>assets/images/banner/webtoffee_logo_transparent.svg') right no-repeat; display:block; clear:both; box-sizing:border-box; }
		            .wt_promotion_banner_inner{ display:inline-block; width:100%; padding:20px 15px; box-sizing:border-box; }
		            .wt_promotion_banner_left{ float:left; width:100px; height:90px; background:#F9F9F9 url('<?php echo esc_url(WT_O_IEW_PLUGIN_URL );?>assets/images/banner/webtoffee_logo.svg') center no-repeat; border-radius:4px;}
		            .wt_promotion_banner_right{ float:left; width: calc( 100% - 100px); padding-left:30px; box-sizing:border-box; }
		            h3.wt_promotion_banner_hd{ font-size:18px; color:#000; font-weight:600; margin:0px; }
		        </style>
