...
class ReviewxPublic extends Controller
{
    ...
    public function __construct($plugin_name, $version)
    {
        ...
        // Review submit admin notification 
        add_action('wp_ajax_nopriv_rx_review_notification_email', array($this, 'rx_review_notification_email'));
        add_action('wp_ajax_rx_review_notification_email', array($this, 'rx_review_notification_email'));
        ...
        add_shortcode('rvx-reviews', array($this, 'reviewx_reviews_list'));

        add_shortcode('rvx_review_google', array($this, 'reviewx_google_review'));
    ...
    public function reviewx_load_order_table()
    {
        $disable_order_table = get_option("_rx_option_review_on_order_table");
        if ($disable_order_table == true) {
            return;
        }
        ...
        add_action('wp_ajax_rx_review_coupon_email', array($this, 'rx_review_coupon_email'));
        add_action('wp_ajax_nopriv_rx_review_coupon_email', array($this, 'rx_review_coupon_email'));
    }
...
?>
    ...
    public function reviewx_google_review()
    {
     ...
     * @return void
     **/
    public function reviewx_get_review_criteria($postdata = array(), $post_type = null)
    {
    ...
    public function reviewx_submit_from_myorder()
    {
        ...
        $forminput              = rest_sanitize_array(wp_unslash($_POST['forminput']));
        $current_user           = wp_get_current_user();
        $comment_agent          = sanitize_textarea_field($_SERVER['HTTP_USER_AGENT']);
        $total_rating_sum       = 0;
        ...
        if ($id) {
            ...
            if(isset($_POST['forminput'])){
                $comment_ex_data = rest_sanitize_array(wp_unslash($_POST['forminput']));
            }
            
            $posts = array(
                'comment_id'    => $id,
                'post_data'     => rest_sanitize_array(wp_unslash($_POST['forminput'])),
                'signature'     => 1
            );
    ...
    public function rx_front_end_review_submit()
    {
        ...
        for ($i = 0; $i < count(wp_unslash($_POST['formInput'])); $i++) {

            if ($forminput[$i]['name'] == "author") :
                $find = array('$', '#', 'file:///etc',':=','+','-','=','@','*','/');
                $replacement = ' ';
                $result = str_replace($find, $replacement,$forminput[$i]['value']);
                $author = sanitize_text_field($result);
            endif;
            ...
            if ($forminput[$i]['name'] == "reviewx_title") :
                ...
                $replacement = ' ';
                $result = str_replace($find, $replacement,$forminput[$i]['value']);
                $review_title = sanitize_text_field($result);
            endif;
        ...
        $comment_data = array(
            ...
            'comment_date'         => date('Y-m-d H:i:s'),
            'comment_date_gmt'     => date('Y-m-d H:i:s'),
            'comment_content'      => str_replace(array('$', '#', 'file:///etc',':=','+','-','=','@','*','/'), ' ',$review_text),
            'comment_karma'        => '',
            'comment_agent'        => $comment_agent,
            'comment_type'         => sanitize_text_field('review'),
            'comment_parent'       => '',
            'user_id'              => $user_id,
        );
        ...
        if ($id) {
            ...
            if(isset($_POST['forminput'])){
                $comment_extra_data  = rest_sanitize_array(wp_unslash($_POST['forminput']));
            }
            $posts = array(
                'comment_id'    => $id,
                'post_data'     => rest_sanitize_array(wp_unslash($_POST['formInput'])),
            ...
            *
            ...
            $customer_ip = \ReviewX_Helper::get_reviewer_ip();
            $response = wp_remote_get('http://ip-api.com/json/' . $customer_ip);
            $result = json_decode($response['body'], true);
            $country_code = '';
            if(isset($result['countryCode'])){
                $country_code = trim($result['countryCode']);
            }
            add_comment_meta($id, 'reviewx_location', $country_code, false);
     ...
     * @return void
     */
    public function rx_review_notification_email()
    {
        $rx_enable_notification = get_option('rx_admin_notification');

        if (class_exists('ReviewXPro') && $rx_enable_notification == 1) {
            $rx_review_id           = absint($_POST['from_data']['commnet_id']);
            $rx_review_title        = get_comment_meta($rx_review_id, 'reviewx_title');
            $rx_avg_rating          = get_comment_meta($rx_review_id, 'rating');
            $rx_review_content      = get_comment($rx_review_id)->comment_content;
            $rx_post                = get_comment($rx_review_id)->comment_post_ID;
            $rx_post_name           = get_post($rx_post)->post_title;
            $rx_post_link           = get_permalink($rx_post);
            $rx_selected_rating     = trim(get_option('rx_selected_notification_rating'));
            $rx_commnet_author      = get_comment_author($rx_review_id);
            $rx_commnet_author_mail = get_comment_author_email($rx_review_id);

            $to                     = get_option('rx_notification_admin_email');
            $subject = "You have a new review for $rx_post_name with a $rx_avg_rating[0] star rating";
            $body = "<tbody>
                                ...
                                <div style='font-size: 16px;'>
                                    <p class='text-extra-large text-center congrats'
                                        style='
                                            ...
                                            mso-line-height-rule: exactly;
                                            font-size: 16px;'
                                    > Reviewer Name: $rx_commnet_author
                                    </p>
                                    ...
                                    <p class='text-extra-large text-center congrats'
                                        style='
                                            ...
                                            mso-line-height-rule: exactly;
                                            font-size: 16px;'
                                    > Reviewer Email: $rx_commnet_author_mail
                                    </p>
                                </div>
                                <div style='font-size: 20px;'>
                                    <p
                                        class='text-extra-large text-center congrats'
                                        style='
                                            -ms-text-size-adjust: 100%;
                                            -webkit-text-size-adjust: 100%;
                                            color: #444;
            ...
            if ($rx_selected_rating == 'all') {
                wp_mail($to, $subject, $body, $headers, array(''));
            }
            if ($rx_selected_rating == 'five' && $rx_avg_rating[0] == 5) {
                wp_mail($to, $subject, $body, $headers, array(''));
            }
            if ($rx_selected_rating == 'four' && $rx_avg_rating[0] > 4) {
                wp_mail($to, $subject, $body, $headers, array(''));
            }
            if ($rx_selected_rating == 'three' && $rx_avg_rating[0] < 3) {
                wp_mail($to, $subject, $body, $headers, array(''));
            }
        }
        wp_send_json(true);
    ...
    public function rx_review_coupon_email()
    {
        ...
        $headers                    = array('Content-Type: text/html; charset=UTF-8');
        $rx_review_id               = absint($_POST['from_data']['commnet_id']);
        $rx_post_id                 = get_comment($rx_review_id)->comment_post_ID;
        $rx_comment_author_name     = get_comment_author($rx_review_id);
        $rx_commnet_author_mail     = get_comment_author_email($rx_review_id);
        $register_user_check        = email_exists($rx_commnet_author_mail);
        $rx_verified_customer       = \ReviewX_Helper::wc_review_is_from_verified_owner($rx_review_id, $rx_post_id);
        $register_user_option       = get_option('register_user');
        ...
        $rx_avg_rating              = get_comment_meta($rx_review_id, 'rating');
        $rx_attachment              = get_comment_meta($rx_review_id, 'reviewx_attachments', true);
        ...
        $rx_coupon_post_id          = \ReviewX_Helper::get_id_by_post_name($rx_coupon_selected_coupon);
        $coupon_epoch_date          = get_post_meta($rx_coupon_post_id, 'date_expires', false);
        // $rx_existing_coupon_date    = date( "d-m-Y", substr($coupon_epoch_date[0], 0, 10) );
        $end_date = substr($coupon_epoch_date[0], 0, 10) ? intval(substr($coupon_epoch_date[0], 0, 10)) : null;
        if ($end_date == null) {
            $rx_existing_coupon_date = '';
        } else {
            $rx_existing_coupon_date    = date("d-m-Y", $end_date);
        }
        if ($review_for_discount == 'on') {

            if ($rx_coupon_type == 'existing_coupon') {
                $email_content = str_replace("[CUSTOMER_NAME]", $rx_comment_author_name, $email_content);
                $email_content = str_replace("[COUPON_CODE]", $rx_coupon_selected_coupon, $email_content);
                $email_content = str_replace("[DATE_EXPIRES]", $rx_existing_coupon_date, $email_content);
            }

            if ($rx_coupon_type == 'generate_new') {
                $rx_new_generated_coupon = \ReviewX_Helper::rx_generate_new_coupon();
                $email_content = str_replace("[CUSTOMER_NAME]", $rx_comment_author_name, $email_content);
                $email_content = str_replace("[COUPON_CODE]", $rx_new_generated_coupon[0], $email_content);
                $email_content = str_replace("[DATE_EXPIRES]", $rx_new_generated_coupon[1], $email_content);
            }

            if (
                $register_user_option == 'on' && $register_user_check &&
                $rx_coupon_verified_user == 'on' && $rx_verified_customer == true &&
                $coupon_photo_review == 'on' && $rx_attachment
            ) {
                if ($coupon_select_rating == 'all') {
                    wp_mail($rx_commnet_author_mail, $rx_coupon_email_title, $email_content, $headers, array(''));
                }
                if (($rx_avg_rating[0] >= $coupon_select_rating) && ($rx_avg_rating[0]  < ($coupon_select_rating + 1))) {
                    wp_mail($rx_commnet_author_mail, $rx_coupon_email_title, $email_content, $headers, array(''));
                }
            }
...
<?php
        ...
        if (class_exists('WooCommerce') && get_post_type($product_id) == 'product') {
            ...
            if (!empty($product)) {
                ...
                if (($product->get_average_rating() == 0)) {
                    $html .= wc_get_rating_html(0.0001, $rating_count);
                    $html .= sprintf(esc_html__('%s', 'reviewx'), ' <span>(' . esc_html(0) . ')</span>');
                }
                $html .= '</div>';
            }
            ...
            if ($rating == 0) {
                $html .= '<div style="display:flex;">';
                $html .= '<div class="rx-product-rating">';
                $html .= '<div class="rx-star-rating" role="img" aria-label="Rated 0 out of 5">';
    ...
    public function reviewx_reviews_list($atts)
    {
        ...
        ob_start();
        include_once REVIEWX_PARTIALS_PATH . 'storefront/shortcode/templates/rvx-reviews-shortcode.php';
        $out = ob_get_clean();
        return $out;
    }
