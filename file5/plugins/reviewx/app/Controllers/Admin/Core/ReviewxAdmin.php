...
use ReviewX\Modules\VersionUpdater;
...
if (!class_exists('ReviewxAdmin')) {
    ...
    class ReviewxAdmin extends Controller
    {
        ...
        public function __construct($plugin_name, $version)
        {
            ...
            add_action('wp_ajax_rx_email_notification', array($this, 'rx_email_notification'));

            add_action('wp_ajax_rx_export_review', array($this, 'rx_export_review'));

            add_action('wp_ajax_rx_coupon_from_submit', array($this, 'rx_coupon_from_submit'));

            /**
             * Google Review from
             */
            add_action('wp_ajax_rx_google_review_from', array($this, 'rx_google_review_from'));
            ...
            // plugin update from 1.4.6 to 1.4.7 compatibility flug
            add_action('upgrader_process_complete', [VersionUpdater::class, 'update_plugin_v147'], 10, 2);
            add_action('plugins_loaded', [VersionUpdater::class, 'delete_email_template']);
        }
        ...
        public function admin_enqueue_scripts($hook)
        {
            $is_enable = (!in_array($hook, [
                ...
                'reviewx_page_rvx-export',
                'reviewx_page_rx-reviewx-google-review',
            ])
            );
            ...
            $js_filter = $this->wp_localize_script_args();
        ...
        public function menu_page()
        {
            ...
            add_submenu_page('rx-admin', __('Export', 'reviewx'), __('Review Export', 'reviewx'), 'delete_users', 'rvx-export', [$this, 'rvx_export']);
            add_submenu_page('rx-admin', __('Review for Discount', 'reviewx'), __('Review for Discount', 'reviewx'), 'delete_users', 'reviewx-discount', [$this, 'reviewx_discount']);
         ...
         * @since    1.5.9
         */
        public function reviewx_discount()
        {
            ...
            include_once REVIEWX_PARTIALS_PATH . 'admin/reviewx-discount.php';
        }
        ...
        public function google_reviews()
        {
            ...
            include_once REVIEWX_PARTIALS_PATH . 'admin/google_reviews.php';
        }
            ...
            foreach ($meta_fields as $meta_key => $meta_field) {
                ...
                if (in_array($meta_key, array_keys($data))) {
                    ...
                    if (isset($meta_field['default'])) {
                        ...
                        if ($meta_key == 'rx_option_allow_profile_img') {
                            $post_data[$meta_key] = '';
                        }
                        ...
                        if (class_exists('ReviewXPro')) {
                            ...
                            if ($meta_key == 'rx_option_display_location') {
                                $post_data[$meta_key] = '';
                            }
                            ...
                            if ($meta_key == 'rx_option_allow_reviewer_name_censor') {
                                $post_data[$meta_key] = '';
                            }
                        }
        ...
        public function save_setting_tab()
        {
            check_ajax_referer('special-string', 'security');
            $data                 = [];
            $rx_setting_tab_arg = rest_sanitize_array(wp_unslash($_POST['rx_tab_field']));
        ...
        protected function duplicate_reviewx($current_url = '')
        {
            ...
            // Duplicating ReviewX
            if (
                isset($_GET['action'], $_GET['page'], $_GET['post'], $_GET['rx_duplicate_nonce'])
        ...
        public function wp_localize_script_args()
        {
        ...
        // ajax callback for email notification
        public function rx_email_notification()
        {
            $admin_notification = $_POST['data'];
            $admin_email        = sanitize_email(trim($_POST['admin_email']));
            $selected_rating    = sanitize_text_field(trim($_POST['rating']));

            if ($admin_notification) {
                update_option('rx_admin_notification', 1);
            } else {
                update_option('rx_admin_notification', 0);
            }
            if ($admin_email) {
                update_option('rx_notification_admin_email', $admin_email);
            }
            if ($selected_rating) {
                update_option('rx_selected_notification_rating', $selected_rating);
            }
        ...
        // ajax callback for export review
        public function rx_export_review()
        {
            $current_user = wp_get_current_user();
            $allowed_roles = array('editor', 'administrator', 'author', 'shop_manager');
            if (array_intersect($allowed_roles, $current_user->roles)) {
                global $wpdb;
                $filter_field_check = isset($_POST['filterValue']) ? (array) $_POST['filterValue'] : array();
                $selected_column_check = isset($_POST['selectedColumns']) ? (array) $_POST['selectedColumns'] : array();
                $change_column_check = isset($_POST['changeColumnName']) ? (array) $_POST['changeColumnName'] : array();

                $filter_field       = array_map('sanitize_text_field', $filter_field_check);
                $selected_column    = array_map('sanitize_text_field', $selected_column_check);
                $change_column      = array_map('sanitize_text_field', $change_column_check);
                $zero = 0;
                $one  = 1;
                $two  = 2;
                $three = 3;
                ...
                $seven = 7;
                $eight = 8;
                $export_limit       = intval($filter_field[$zero]);
                $skip_export_row    = intval($filter_field[$one]);
                $start_date         = strval($filter_field[$two]);
                $end_date           = strval(date('Y-m-d', strtotime($filter_field[$three] . '+1 day')));
                $product_name       = strval($filter_field[$four]);
                $selected_rating    = isset($filter_field[$five]) ? (string)$filter_field[$five] : '';
                $review_status      = isset($filter_field[$six]) ? (string)$filter_field[$six] : '';
                $review_sort        = isset($filter_field[$seven]) ? (string)$filter_field[$seven] : '';
                $review_sort_by     = isset($filter_field[$eight]) ? (string)$filter_field[$eight] : '';
                $comment_data   = "comment_date";
                $comment_prefix = "{$wpdb->prefix}comments";
                $first_commnet_date = json_decode(json_encode($wpdb->get_results(
                    $wpdb->prepare("SELECT 
                    `$comment_data` FROM `$comment_prefix` 
                    ORDER BY `$comment_data` ASC LIMIT 1")
                )), true);
                $first_commnet_date = strtok($first_commnet_date[0]['comment_date'], " ");
                ...
                if (!$start_date) {
                    $start_date = strval($first_commnet_date);
                }
                if (!$end_date) {
                    $end_date = strval(date('Y-m-d', strtotime("+1 day")));
                }


                $review_saperated_value = \ReviewX_Helper::selected_export_column_value($selected_column);
                $comment_table_columns  = $review_saperated_value[0];
                $selected_meta_row      = $review_saperated_value[1];


                if ($comment_table_columns[0] == 'all') {
                    array_shift($comment_table_columns);
                    array_shift($change_column);
                }

                $sql_column_name = implode(',', $comment_table_columns);
                $comments = "{$wpdb->prefix}comments";
                if ($product_name == 'all') {
                    $review_type = 'review';
                    $sql_query = $wpdb->prepare(
                        "SELECT * FROM `$comments` AS comments
                    INNER JOIN {$wpdb->prefix}posts AS post 
                    ON post.ID = comments.comment_post_ID 
                    WHERE ( comments.comment_type = %s AND comments.comment_parent = 0
                        AND comments.comment_approved = %s
                        AND (comments.comment_date_gmt BETWEEN %s AND %s ))
                ...
                // var_dump($sql_query );
                $comment_data = $wpdb->get_results($sql_query);
                ...
                foreach ($comment_data as $key => $comments) {
                    ...
                    $stractured_data = array_intersect_key($stractured_data, array_flip($selected_meta_row));

                    $review_data[] = $comments + $stractured_data;
                }

                $file_name = 'reviews' . time() . '.csv';
                $file_csv = fopen($file_name, 'w+');
                fputcsv($file_csv, $change_column);
                foreach ($review_data as $key => $value) {
                    ...
                    if (array_key_exists("rating", $value)) {
                        ...
                        if (($value["rating"] >= $selected_rating) && ($value["rating"]  < ($selected_rating + 1))) {
                            fputcsv($file_csv, $value);
                        }
                    }
                }
        ...
        // coupon from submit ajax
        public function rx_coupon_from_submit()
        {
            if (!wp_verify_nonce($_REQUEST['rx_coupon_id'], 'create')) {
                return;
            }
            if (!current_user_can('manage_options')) {
                wp_die("You are not authorized");
            }
            $rx_all_coupon_data = $_POST['rx_all_coupon_data'];
            $rx_selected_coupon_from_data = $_POST['rx_selected_coupon_from_data'];

            $rx_not_selected_field = array_diff_key($rx_all_coupon_data, $rx_selected_coupon_from_data);

            foreach ($rx_selected_coupon_from_data as $key => $data) {
                update_option($key, $data);
            }

            foreach ($rx_not_selected_field as $key => $data) {
                update_option($key, 'off');
            }

            if (!array_key_exists('rx_coupon_cat_name', $rx_all_coupon_data)) {
                update_option('rx_coupon_cat_name', []);
            }

            if (!array_key_exists('rx_coupon_product_name', $rx_all_coupon_data)) {
                update_option('rx_coupon_product_name', []);
            }
            wp_send_json(true);
            wp_die();
        }
