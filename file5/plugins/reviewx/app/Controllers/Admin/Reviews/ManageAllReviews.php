...
class ManageAllReviews extends WordpressListTable
{
    ...
    {
        global $wpdb;
        $results = $wpdb->get_results( "SELECT comment_author FROM {$wpdb->prefix}comments 
            WHERE comment_type = 'review'
    ...
    {
        echo '<input type="hidden" class="rx-product-id-input" name="productId" value=""/>';
        ...
        if( $_POST && $_POST["productId"] ){
            ...
            $totalReviews = count(get_comments(array(
                'post_id' => $product_id )));
            ...
            $averageRating = ( $total_ratings / $totalReviews );
            $totalProducts = number_format($averageRating,2);
    ...
    {
        global $comment_status, $search, $product_id;
        ...
        foreach ($comments as $comment) {
            ...
            if ( current_user_can( 'edit_post', $post_id ) ) {
                $post_link  = "<a class='rx-single-product' href='#' data-id='$post_id'>";
    ...
    public function column_default( $item, $column_name )
    {
        switch( $column_name ) {
            ...
            case 'avg_rating':
                ...
                $owner_rating = null;
                if( $item['parent_id'] != 0 ){
                    return \view('partials.utilities.store-owner-rating',compact('owner_rating', 'domain'));
                } else {
            ...
            case 'status':
                if ($item[$column_name] == 1) {
                    $review_id = $item['id'];
                    return \view('partials.utilities.approved', compact('review_id'));
                } else {
                    $review_id = $item['id'];
                    return \view('partials.utilities.unapproved', compact('review_id'));
                }
    ...
    public function deduct_rating_by_previous_comment_status($comment_id, $action)
    {
        ...
        if ( ! empty($mapStatusPrevent[$comment_status]) && ! in_array($action, $mapStatusPrevent[$comment_status])) {
    ...
    public function review_action_status()
    {
        ...
        if ( isset( $_REQUEST['approved'] ) || isset( $_REQUEST['deleted'] ) || isset( $_REQUEST['trashed'] ) || isset( $_REQUEST['untrashed'] ) || isset( $_REQUEST['spammed'] ) || isset( $_REQUEST['unspammed'] ) || isset( $_REQUEST['same'] ) ) {
            ...
            if ( $approved > 0 || $deleted > 0 || $trashed > 0 || $untrashed > 0 || $spammed > 0 || $unspammed > 0 || $same > 0 ) {
                ...
                if ( $spammed > 0 ) {
                    $ids = absint(isset( $_REQUEST['ids'] ) ? $_REQUEST['ids'] : 0);
                    /* translators: %s: Number of comments. */
        ...
        if ( $comment->comment_parent ) {
            ...
            if ( $parent ) {
                ...
                /* translators: %s: Comment link. */
                    ...
                    '<a href="' . esc_url($parent_link) . '">' . $name . '</a>'
                );
            }
        }
    ...
    protected function handle_row_actions( $comment, $column_name, $primary )
    {
        ...
        if ( $comment_status && 'all' != $comment_status ) {
            ...
            $actions['approve'] = sprintf(
                '<a href="%s" data-wp-lists="%s" class="vim-a aria-button-if-js" aria-label="%s" onClick="window.location.reload();">%s</a>',
                $approve_url,
                "dim:the-comment-list:comment-{$comment->comment_ID}:e7e7d3:action=dim-comment&amp;new=approved",
                esc_attr__( 'Approve this comment','reviewx' ),
                __( 'Approve', 'reviewx' )
            );

            $actions['unapprove'] = sprintf(
                '<a href="%s" data-wp-lists="%s" class="vim-u aria-button-if-js" aria-label="%s" onClick="window.location.reload();">%s</a>',
                $unapprove_url,
                "dim:the-comment-list:comment-{$comment->comment_ID}:e7e7d3:action=dim-comment&amp;new=unapproved",
                esc_attr__( 'Unapprove this comment', 'reviewx' ),
                __( 'Unapprove', 'reviewx' )
            );
        }
