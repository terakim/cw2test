...
class ReviewX_Notice {
    ...
    public function __construct( $plugin_file = '', $version = '' ) {
        ...
        if( ! class_exists( 'ReviewX_Core_Installer' ) ) {
            require_once dirname(__FILE__) . '/class-rx-core-installer.php';
            new ReviewX_Core_Installer( $this->plugin_name );
        }
    }
    /**
     * Initiate The Plugin
    ...
    public function hooks(){
        ...
        add_action( 'wp_ajax_second_notice_dismiss', array( $this, 'second_notice_dismiss' ) );
    ...
    private function upsale_button(){
        ...
        echo '<button data-slug="'. $plugin_slug .'" id="plugin-install-core-'. $this->plugin_name .'" class="button button-primary">'. __( 'Install Now!', 'reviewx' ) .'</button>';
    }
    /**
...
//                $output .= '<img src="'. $this->data['thumbnail'][ $msg_for ] .'" alt="ReviewX">';
//            $output .= '</div>';
//        }
        echo $output;
    }
    ...
    public function second_notice_dismiss() {
        if( isset($_POST) ){
            update_option('rvx_second_cupon', $_POST['data']);
        }
    }
...
// Initialization.
$notice = new ReviewX_Notice(REVIEWX_BASENAME, REVIEWX_VERSION);
$scheme = (parse_url( $_SERVER['REQUEST_URI'], PHP_URL_QUERY )) ? '&' : '?';
$url = $_SERVER['REQUEST_URI'] . $scheme;
$notice->links = [
    'review' => array(
        'later' => array(
            'link' => esc_url('https://wpdeveloper.net/review-reviewx'),
            'target' => '_blank',
            'label' => __( 'Ok, you deserve it!', 'reviewx' ),
            'icon_class' => 'dashicons dashicons-external',
        ),
        'allready' => array(
            'link' => $url,
            'label' => __( 'I already did', 'reviewx' ),
        ...
        'support' => array(
            'link' => esc_url('https://wpdeveloper.net/support'),
            'label' => __( 'I need help', 'reviewx' ),
            'icon_class' => 'dashicons dashicons-sos',
        ),
        'never_show_again' => array(
            'link' => $url,
            'label' => __( 'Never show again', 'reviewx' ),
...
$notice->message( 'upsale', '<p>'. __( 'Get the missing feature for ReviewX by installing PRO!', 'reviewx' ) .'</p>' );
$notice->thumbnail( 'upsale', plugins_url( 'resources/assets/admin/images/ReviewX.svg', REVIEWX_BASENAME ) );

/**
 * This is review message and thumbnail.
 */
$notice->message( 'review', '<p>'. __( 'We hope you\'re enjoying ReviewX! Could you please do us a BIG favor and give it a 5-star rating on WordPress to help us spread the word and boost our motivation?', 'reviewx' ) .'</p>' );
$notice->thumbnail( 'review', plugins_url( 'resources/assets/admin/images/ReviewX.svg', REVIEWX_BASENAME ) );
