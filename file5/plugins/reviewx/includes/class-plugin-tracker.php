...
/**
 * Main SDK for ReviewX_Plugin_Tracker.
 */
if (!class_exists('ReviewX_Plugin_Tracker')) :
	class ReviewX_Plugin_Tracker
	{
		...
		/**
		 * Instace of ReviewX_Plugin_Tracker
		 * @var ReviewX_Plugin_Tracker
		 */
		private static $_instance = null;
		/**
		 * Get Instance of ReviewX_Plugin_Tracker
		 * @return ReviewX_Plugin_Tracker
		 */
		...
		public function init()
		{
			...
			add_action('admin_notices', array($this, 'new_notice'));
			...
			add_action('wp_ajax_second_notice_action', array($this, 'second_notice_action'));
		}
		/**
		...
		{
			$notice = get_option('wpins_block_notice');
			$second_notice = get_option('rvx_second_cupon');
			if (!$notice && !isset($notice[$this->plugin_name])) return;

			if ($second_notice == false) {
				if (!\ReviewX_Helper::is_pro()) {
					$output = '';
					$output .= '<div class="second-notice-bar notice notice-success is-dismissible" style="display:flex; height: 60px;">';
					$output .= '<p class="second-notice-bar-text" style="padding: 12px 0px 10px 0px;">Get exclusive <strong> 30% </strong> OFF with coupon code <strong><i> ‘RvXSave30’  </i> </strong> while upgrading to <strong > ReviewX PRO</strong></p>';
					$output .= '<p style="padding: 8px;">';
					$output .= '<a href="' . esc_url('https://reviewx.io/') . '" class="button-secondary" target="_blank"> Upgrade to Pro </a>';
					$output .= '</div>';
					echo $output;
				}
			}
		...
		public function notice()
		{
			...
			$nonce = wp_create_nonce('rvx_nonce_action');
			wp_nonce_field('rvx_nonce_action', 'rvx_nonce_field');

			$url_yes = add_query_arg([
				'plugin'          => $this->plugin_name,
				'plugin_action'   => 'yes',
				'action' => 'rvx_nonce_action',
				'nonce'  => $nonce,
			]);
		...
		{
			$default_options = [
				'consent_button_text' => __('What we collect.', 'reviewx'),
				'yes'                 => __('Sure, I\'d like to help', 'reviewx'),
				'no'                  => __('No Thanks.', 'reviewx'),
			];
		...
		public function deactivate_reasons_form()
		{
			...
			$html .= '<p class="deactivating-spinner"><span class="spinner"></span> ' . __('Submitting form', 'reviewx') . '</p>';
			...
			<script type="text/javascript">
				jQuery(document).ready(function($) {
							$("#wpinsights-goodbye-link-<?php echo $class_plugin_name; ?>").on("click", function() {
								...
								$(".wpinsights-goodbye-form-wrapper-<?php echo $class_plugin_name; ?> #wpinsights-goodbye-form").html('<?php echo $html; ?>' + '<div class="wpinsights-goodbye-form-footer"><div class="wpinsights-goodbye-form-buttons"><a id="wpinsights-submit-form-<?php echo $class_plugin_name; ?>" class="wpinsights-submit-btn" href="#"><?php _e('Submit and Deactivate', 'reviewx'); ?></a>&nbsp;<a class="wpsp-put-deactivate-btn" href="' + url + '"><?php _e('Just Deactivate', 'reviewx'); ?></a></div></div>');
