...
if ( ! class_exists( 'MSPS_Manager' ) ) {
	...
	class MSPS_Manager {
		...
		protected static $_use_print_notice_archive = null;
			...
			switch ( $type ) {
				...
				case 'cart' :
					if ( null == self::$_use_print_notice_cart ) {
						self::$_use_print_notice_cart = ( 'yes' == get_option( 'mshop_point_system_use_print_guide_notice_product', 'no' ) );
					}

					return self::$_use_print_notice && self::$_use_print_notice_cart;
				case 'checkout' :
					if ( null == self::$_use_print_notice_checkout ) {
						self::$_use_print_notice_checkout = ( 'yes' == get_option( 'mshop_point_system_use_print_guide_notice_product', 'no' ) );
					}
				...
				case 'archive' :
					if ( null == self::$_use_print_notice_archive ) {
						self::$_use_print_notice_archive = ( 'yes' == get_option( 'msps_use_print_notice_archive', 'no' ) );
					}

					return self::$_use_print_notice && self::$_use_print_notice_archive;
			}
		...
		public static function point_exchange_ratio() {
			return get_option( 'mshop_point_system_point_exchange_ratio', '1' );
		}
		...
		public static function max_useable_amount( $cart ) {
			...
			if ( self::allow_pay_shipping() ) {
				...
				$shipping_total += apply_filters( 'msiv_get_iv_shipping_fee', 0, $cart );

				$useable_amount += $shipping_total;
			}
			...
			foreach ( $cart->get_cart() as $cart_item ) {
				...
				if ( ! $product->is_point_purchasable() ) {
					if ( wc_tax_enabled() && wc_prices_include_tax() ) {
						$useable_amount -= ( $cart_item['line_total'] + $cart_item['line_total_tax'] );
					} else {
						$useable_amount -= $cart_item['line_total'];
					}
				}
			}

			if ( wc_tax_enabled() && wc_prices_include_tax() ) {
				if ( $useable_amount > ( $cart->get_cart_contents_total() + $cart->get_cart_contents_tax() + $shipping_total ) ) {
					$useable_amount = $cart->get_cart_contents_total() + $cart->get_cart_contents_tax() + $shipping_total;
				}
			} else {
				if ( $useable_amount > ( $cart->get_cart_contents_total() + $shipping_total ) ) {
					$useable_amount = $cart->get_cart_contents_total() + $shipping_total;
				}
			}
				...
				$args = array(
					...
					'tax_query'      => array(
						array(
							...
							'field'    => 'slug',
							'terms'    => apply_filters( 'msps_point_rule_terms', self::$terms )
						),
					),
				);
		...
		public static function update_point_rule_meta( $point_rule_id, $data ) {
			...
			update_post_meta( $point_rule_id, '_exclude_sales', $data['exclude_sales'] );
		...
		public static function get_expected_point_from_product_rule( &$cart_items, $user_role ) {
			...
			foreach ( $cart_items as $cart_item ) {
				...
				if ( $product->enabled() ) {
					...
					if ( $rule ) {
						$total += $product->calculate_point( $quantity, $user_role, $cart_item );
					}
			...
			$cart_items = array_filter( $cart_items, function ( $cart_item ) use ( $applied_ids ) {
				...
				return ! in_array( $product->get_id(), $applied_ids );
			} );

			return $total;
		}
		protected static function get_expected_point_from_global_rule( $rules, $cart_items, $filter, $user_role, $order ) {
			$total = 0;
				...
				foreach ( $rules as $rule ) {
					...
					foreach ( $cart_items as $cart_item ) {
						...
						if ( $rule->is_match( $product ) ) {
							...
							$rule->add_item( $product, $quantity, $cart_item );
						}
					}
					if ( $rule->is_applicable( $order ) ) {
						$total += $rule->calculate_point( $user_role, $order );
		...
		public static function get_expected_point( $object, $qty, $user_role ) {
			...
			if ( ( is_user_logged_in() || $user_id > 0 ) && self::is_valid_user( $user_role ) ) {
				self::get_point_rules();
				...
				if ( $object instanceof WC_Product ) {
					...
					foreach ( $object->get_items() as $item ) {
						...
						if ( $product && apply_filters( 'msps_order_item_is_eligible_for_earn_point', true, $item ) ) {
				...
				$purchase_items = array_filter( $purchase_items, function ( $cart_item ) {
					...
					return ( msps_get( $cart_item, 'total', 0 ) > 0 || $product->get_price() > 0 ) && ! $point_product->is_except_earn_point();
				} );

				if ( count( $purchase_items ) > 0 ) {
					$items = $purchase_items;
					$total = self::get_expected_point_from_product_rule( $items, $user_role );
					$total += self::get_expected_point_from_global_rule( self::$point_seperate_rules, $items, true, $user_role, $order );
					$total += self::get_expected_point_from_global_rule( self::$point_always_rules, $purchase_items, false, $user_role, $order );
				}
			}
			...
			if ( $product->enabled() ) {
				foreach ( $product->active_rules as $price_rule ) {
					...
					if ( ! empty( $option ) ) {
						...
						$option = apply_filters( 'msps_point_option', $option, $user_role, get_current_user_id() );

						$ruleset[] = array(
							'amount'     => floatval( $price_rule['amount'] ),
							'qty'        => intval( $price_rule['qty'] ),
							'fixed'      => intval( $option['fixed'] ),
							'fixed_text' => number_format_i18n( floatval( $option['fixed'] ), wc_get_price_decimals() ),
				...
				foreach ( $rules as $rule ) {
					...
					if ( $rule->is_match( $product ) ) {
						foreach ( $rule->price_rules as $price_rule ) {
							...
							$option = apply_filters( 'msps_filter_price_rule_options', $option, $price_rule );
		...
		public static function show_message_for_product( $product, $qty, $user_id, $place = 'product_detail' ) {
			if ( ! apply_filters( 'msps_price_notice', true, $place ) ) {
				return '';
			}
			...
			if ( $point > 0 ) {
				...
				$result[] = str_replace( '{point}', number_format_i18n( $point, wc_get_price_decimals() ), $message );
			}

			if( self::use_print_notice( $place ) ) {
						...
						if ( ! empty( $rule['option']['fixed'] ) || ! empty( $rule['option']['ratio'] ) ) {
							...
							if ( $rule['option']['ratio'] > 0 ) {
								...
								if ( $rule['option']['fixed'] > 0 ) {
									$point .= ' + ' . number_format_i18n( $rule['option']['fixed'], wc_get_price_decimals() );
								}
							} else {
								$point = ! empty( $rule['option']['fixed'] ) ? number_format_i18n( $rule['option']['fixed'], wc_get_price_decimals() ) : '';
							}
