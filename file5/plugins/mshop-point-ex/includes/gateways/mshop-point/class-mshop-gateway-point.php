...
class MShop_Gateway_Point extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_payment_complete_order_status', array( $this, 'woocommerce_payment_complete_order_status' ), 15, 3 );
	}
	...
	public function woocommerce_review_order_before_submit() {
		$user        = new MSPS_User( get_current_user_id() );
		$point       = number_format( $user->get_point(), wc_get_price_decimals() );
	...
	public function payment_fields() {
		...
		echo sprintf( __( '<p>보유포인트 : %s</p>', 'mshop-point-ex' ), number_format_i18n( $user->get_point(), wc_get_price_decimals() ) );
	}
	public function woocommerce_payment_complete_order_status( $order_status, $order_id, $order = null ) {
		...
		if ( $this->id == $order->get_payment_method() ) {
			$order_status = get_option( 'mshop_point_system_order_status_after_payment', $order_status );
		}
	...
	public function process_payment( $order_id ) {
		...
		$order->add_order_note( sprintf( __( '포인트(%s)를 사용해서 결제했습니다.', 'mshop-point-ex' ), number_format( $used_point, wc_get_price_decimals() ) ) );

		MSPS_Log::add_log( get_current_user_id(), msps_get_wallet_id( 'free_point', $order ), 'deduct', 'payment', - 1 * $used_point, $remain_point, 'completed', $order_id );

		$message = sprintf( __( '주문(<a href="%s">#%s</a>) 결제시 포인트 사용으로 %s포인트가 차감되었습니다.<br>보유포인트가 %s포인트에서 %s포인트로 변경되었습니다.', 'mshop-point-ex' ), get_edit_post_link( $order_id ), $order_id, number_format( floatval( $used_point ), wc_get_price_decimals() ), number_format( $prev_point, wc_get_price_decimals() ), number_format( $remain_point, wc_get_price_decimals() ) );
