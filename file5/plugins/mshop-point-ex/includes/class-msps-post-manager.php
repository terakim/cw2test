...
if ( ! class_exists( 'MSPS_Post_Manager' ) ) {
	...
	class MSPS_Post_Manager {
		...
		protected static function update_point_rule_meta( $point_rule_id, $data ) {
			...
			update_post_meta( $point_rule_id, '_post_length', $data['post_length'] );
			update_post_meta( $point_rule_id, '_comment_length', $data['comment_length'] );
		...
		public static function get_expected_comment_point( $post_id, $user_role, $comment ) {
			self::get_point_rules();

			foreach ( self::$point_rules as $rule ) {
				if ( $rule->is_valid() && $rule->is_match( $post_id ) ) {
					$min_comment_length = intval( $rule->comment_length );

					if ( 0 == $min_comment_length || $min_comment_length <= mb_strlen( $comment->comment_content ) ) {
		...
		public static function get_expected_post_point( $post_id, $user_role, $post ) {
			self::get_point_rules();

			foreach ( self::$point_rules as $rule ) {
				if ( $rule->is_valid() && $rule->is_match( $post_id ) ) {
					$min_post_length = intval( $rule->post_length );

					if ( 0 == $min_post_length || $min_post_length <= mb_strlen( $post->post_content ) ) {
					...
					foreach ( self::$point_rules as $rule ) {
						if ( $rule->is_valid() && $rule->is_match( $post_id ) && self::can_earn_point( get_current_user_id(), $post_id, 'comment' ) ) {
							...
							if ( ! empty( $point ) && intval( $point ) > 0 ) {
								...
								$message = str_replace( '{point}', number_format_i18n( $point, wc_get_price_decimals() ), $message );
		...
		static function get_post_today_earn_count( $user_id ) {
			return self::get_post_earn_count( $user_id, date( "Y-m-d 00:00:00", strtotime( current_time( 'mysql' ) ) ), date( "Y-m-d 23:59:59", strtotime( current_time( 'mysql' ) ) ) );
		}
		protected static function get_comment_today_earn_count( $user_id, $post_id ) {
			return self::get_comment_earn_count( $user_id, $post_id, date( "Y-m-d 00:00:00", strtotime( current_time( 'mysql' ) ) ), date( "Y-m-d 23:59:59", strtotime( current_time( 'mysql' ) ) ) );
		}
		protected static function get_today_earn_point( $user_id ) {
			return MSPS_Post_Manager::get_earn_point( $user_id, date( "Y-m-d 00:00:00", strtotime( current_time( 'mysql' ) ) ), date( "Y-m-d 23:59:59", strtotime( current_time( 'mysql' ) ) ) );
		}
		protected static function get_weekly_earn_point( $user_id ) {
			$day_of_week = date( 'N', strtotime( current_time( 'mysql' ) ) );
			$from_date   = new DateTime( date( "Y-m-d", strtotime( current_time( 'mysql' ) ) ) . " -" . ( $day_of_week - 1 ) . " day" );

			return MSPS_Post_Manager::get_earn_point( $user_id, $from_date->format( 'Y-m-d 00:00:00' ), date( "Y-m-d 23:59:59", strtotime( current_time( 'mysql' ) ) ) );
		}
		protected static function get_monthly_earn_point( $user_id ) {
			return MSPS_Post_Manager::get_earn_point( $user_id, date( "Y-m-01 00:00:00", strtotime( current_time( 'mysql' ) ) ), date( "Y-m-d 23:59:59", strtotime( current_time( 'mysql' ) ) ) );
		}
		public static function get_comment_earn_count( $user_id, $post_id, $from, $to ) {
			global $wpdb;

			$from = apply_filters( 'msps_comment_earn_count_from', $from, $user_id, $post_id );
			$to   = apply_filters( 'msps_comment_earn_count_to', $to, $user_id, $post_id );
		...
		public static function get_post_earn_count( $user_id, $from, $to ) {
			...
			$from = apply_filters( 'msps_post_earn_count_from', $from, $user_id );
			$to   = apply_filters( 'msps_post_earn_count_to', $to, $user_id );
