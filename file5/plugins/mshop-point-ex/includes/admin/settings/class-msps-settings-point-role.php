...
if ( ! class_exists( 'MSPS_Settings_Point_Role' ) ) :
    ...
    class MSPS_Settings_Point_Role {
        ...
        public static function output(){
	        ...
	        $license_info = json_decode( get_option( 'msl_license_' . MSPS()->slug(), json_encode( array(
		        'slug'   => MSPS()->slug(),
		        'domain' => preg_replace( '#^https?://#', '', home_url() )
	        ) ) ), true );

	        $license_info = apply_filters( 'mshop_get_license', $license_info, MSPS()->slug() );
            wp_localize_script( 'mshop-setting-manager', 'mshop_setting_manager', array(
                'element' => 'mshop-setting-wrapper',
                'ajaxurl' => admin_url( 'admin-ajax.php' ),
                'action' => msps_ajax_command( 'update_msps_role_settings' ),
                'settings' => $settings,
                'slug' => MSPS()->slug(),
            ...
            <script>
                jQuery(document).ready(function(){
                    jQuery(this).trigger('mshop-setting-manager', [ 'mshop-setting-wrapper', '100', <?php echo json_encode( MSSHelper::get_settings( $settings ) ); ?>, <?php echo json_encode( $license_info );; ?>  ]);
                });
            </script>
