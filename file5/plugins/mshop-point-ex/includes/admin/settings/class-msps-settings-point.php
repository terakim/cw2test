...
if ( ! class_exists( 'MSPS_Settings_Point' ) ) :
	...
	class MSPS_Settings_Point {
		...
		static function init() {
			...
			add_filter( 'msshelper_get_msps_visit_point_rule', array( __CLASS__, 'get_visit_point_rule_value' ) );
			add_filter( 'msshelper_get_msps_sms_point_rule', array( __CLASS__, 'get_sms_point_rule_value' ) );
		}
		...
		static function get_visit_point_rule_value( $value ) {
			$rules = array_combine( array_column( $value, 'role' ), $value );

			$filters = get_option( 'mshop_point_system_role_filter' );

			foreach ( $filters as $role ) {
				if ( 'yes' != $role['enabled'] ) {
					unset( $rules[ $role['role'] ] );
				} else if ( empty( $rules[ $role['role'] ] ) ) {
					$rules[ $role['role'] ] = array(
						'role'  => $role['role'],
						'name'  => $role['name'],
						'day'   => 0,
						'week'  => 0,
		...
		static function get_sms_point_rule_value( $value ) {
			$rules = array_combine( array_column( $value, 'role' ), $value );
		...
		static function get_mshop_point_rules( $rules ) {
			...
			foreach ( MSPS_Manager::get_point_rules() as $point_rule ) {
				$point_rules[] = array(
					...
					'exclude_sales'  => $point_rule->exclude_sales,
		...
		static function get_mshop_point_post_rules( $rules ) {
			...
			foreach ( MSPS_Post_Manager::get_point_rules() as $point_rule ) {
				$point_rules[] = array(
					...
					'use_valid_term' => $point_rule->use_valid_term,
					'valid_term'     => $point_rule->valid_term,
					'post_length'    => ! empty( $point_rule->post_length ) ? $point_rule->post_length : 0,
					'comment_length' => ! empty( $point_rule->comment_length ) ? $point_rule->comment_length : 0
				);
			}
					...
					array(
						...
						'elements' => array(
							array(
								...
								"type"      => "Toggle",
								"default"   => "no",
								"desc"      => __( "<div class='desc2'>포인트 적립 안내 문구 출력 기능을 사용합니다.</div>", 'mshop-point-ex' )
							)
						)
					),
					array(
						...
						'elements' => array(
							array(
								...
								"type"      => "Toggle",
								"default"   => "yes",
								"desc"      => __( "<div class='desc2'>상품 구매시 적립될 포인트에 대한 안내 문구를 출력합니다.</div>", 'mshop-point-ex' )
							),
							array(
								'id'      => 'msps_notice_product_hook',
								'showIf'  => array( 'mshop_point_system_use_print_notice_product' => 'yes' ),
								'title'   => __( '적립정보 표시위치', 'mshop-point-ex' ),
								"className" => "fluid",
								'default' => 'woocommerce_after_add_to_cart_form',
								'type'    => 'Text',
								'desc2'   => __( '<div class="desc2">포인트 적립정보를 표시할 위치(Hook)를 지정합니다.</div>', 'mshop-point-ex' )
							),
							array(
								...
								'type'    => 'TextArea',
								'desc2'   => __( '<div class="desc2">기본문구는 "상품 구매시 {point} 포인트가 적립됩니다." 입니다.<br>{point} : 적립포인트</div>', 'mshop-point-ex' )
							),
							array(
								...
								'type'    => 'TextArea',
								'desc2'   => __( '<div class="desc2">기본문구는 "주문이 완료되면 {point} 포인트가 적립됩니다." 입니다.<br>{point} : 적립포인트</div>', 'mshop-point-ex' )
							),
							array(
							...
							array(
								'id'      => 'mshop_point_system_notice_for_quest',
								'showIf'  => array( 'mshop_point_system_use_print_notice_product' => 'yes' ),
								'title'   => __( '비회원 안내 문구', 'mshop-point-ex' ),
								'default' => __( '회원 가입 후 포인트 적립 혜택을 누리세요.', 'mshop-point-ex' ),
								'type'    => 'TextArea',
							),
							array(
							...
							array(
								...
								'type'    => 'TextArea',
								'desc2'   => __( '<div class="desc2">기본문구는 "{desc} 상품 {amount} 이상 구매시 {point} 포인트가 적립됩니다." 입니다.<br>{desc} 상품정보, {amount} : 구매 금액, {point} : 적립포인트</div>', 'mshop-point-ex' )
							),
							array(
								...
								'type'    => 'TextArea',
								'desc2'   => __( '<div class="desc2">기본문구는 "{desc} 상품 {qty}개 이상 구매시 {point} 포인트가 적립됩니다." 입니다.<br>{desc} 상품정보, {qty} : 구매 수량, {point} : 적립포인트</div>', 'mshop-point-ex' )
							),
							array(
								...
								'type'    => 'TextArea',
								'desc2'   => __( '<div class="desc2">기본문구는 "{desc} 상품 {amount} 이상 또는 {qty}개 이상 구매시 {point} 포인트가 적립됩니다." 입니다.<br>{desc} 상품정보, {amount} : 구매 금액, {qty} : 구매 수량, {point} : 적립포인트</div>', 'mshop-point-ex' )
							),
						)
					),
					array(
						'type'     => 'Section',
						'title'    => __( '상품 목록', 'mshop-point-ex' ),
						'showIf'   => array( 'mshop_point_system_use_print_notice' => 'yes' ),
						'elements' => array(
							array(
								"id"        => "msps_use_print_notice_archive",
								"title"     => __( "포인트 적립정보 표시", 'mshop-point-ex' ),
								"className" => "",
								"type"      => "Toggle",
								"default"   => "no",
								"desc"      => __( "<div class='desc2'>상품 목록 페이지에 적립될 포인트에 대한 안내 문구를 출력합니다.</div>", 'mshop-point-ex' )
							),
							array(
								'id'      => 'msps_print_notice_archive_message',
								'showIf'  => array( 'msps_use_print_notice_archive' => 'yes' ),
								'title'   => __( '적립안내 메시지', 'mshop-point-ex' ),
								'default' => __( '상품 구매시 {point} 포인트가 적립됩니다.', 'mshop-point-ex' ),
								'type'    => 'TextArea',
								'desc2'   => __( '<div class="desc2">기본문구는 "상품 구매시 {point} 포인트가 적립됩니다." 입니다.<br>{point} : 적립포인트</div>', 'mshop-point-ex' )
							)
				...
				'elements' => array(
					array(
						...
						'elements' => array(
							...
							array(
								'id'        => 'msps_form_use_point_template',
								'title'     => __( '포인트 사용 템플릿', 'mshop-point-ex' ),
								'className' => '',
								'type'      => 'Select',
								'default'   => 'type-a',
								'options'   => array(
					...
					array(
						...
						'elements' => array(
							...
							array(
								...
								"title"     => __( "포인트 사용단위", 'mshop-point-ex' ),
								"className" => "",
								"type"      => "Text",
								"desc"      => __( "포인트 사용량을 제한합니다.", 'mshop-point-ex' )
							),
							array(
								'id'        => 'msps_cannot_use_point_with_coupons',
								'title'     => __( '쿠폰 사용시 포인트 사용불가', 'mshop-point-ex' ),
								'className' => '',
								'type'      => 'Toggle',
								'default'   => 'no',
								"desc"      => __( '<div class="desc2">쿠폰 사용시 포인트 할인을 받을 수 없습니다.</div>', 'mshop-point-ex' ),
							),
						)
					),
					array(
						...
						'elements' => array(
							array(
								...
								"type"      => "Toggle",
								"default"   => "no",
								"desc"      => __( '<div class="desc2">고객이 결제 취소 또는 환불시 결제에 사용된 포인트를 재적립 합니다.</div>', 'mshop-point-ex' ),
							),
							array(
								...
								"type"      => "Toggle",
								"default"   => "no",
								"desc"      => __( '<div class="desc2">포인트 할인을 받은 경우에도 포인트를 적립합니다.</div>', 'mshop-point-ex' ),
							),
							array(
								...
								"type"      => "Toggle",
								"default"   => "no",
								"desc"      => __( '<div class="desc2">포인트 결제건에 대해서도 포인트를 적립합니다.</div>', 'mshop-point-ex' ),
							),
							array(
								...
								'type'      => 'Toggle',
								'default'   => 'no',
								"desc"      => __( '<div class="desc2">쿠폰을 사용한 주문건에 대해서도 포인트를 적립합니다.</div>', 'mshop-point-ex' ),
							),
							array(
								'id'        => 'mshop_point_reduce_earn_point',
								'title'     => __( '할인 시 적립 포인트 차감', 'mshop-point-ex' ),
								'className' => '',
								'type'      => 'Toggle',
								'default'   => 'no',
								"desc2"     => __( '<div class="desc2">결제시 사용한 쿠폰 및 포인트의 할인 금액에 따라 적립 예정 포인트도 차감합니다.<br>예를 들어, 결제금액이 10,000원, 적립예정 포인트가 100포인트일때, 쿠폰 또는 포인트 할인으로 2,000원을 할인받은 경우, 실제 적립되는 포인트는 80 포인트가 됩니다.</div>', 'mshop-point-ex' ),
							)
						)
					),
					array(
					...
					array(
						...
						'elements' => array(
							array(
								...
								'type'    => 'TextArea',
								'desc2'   => __( '숏코드 : [msps_point_info]<br>기본문구는 "{name} 고객님은 {point} 포인트가 있습니다." 입니다.<br>{name} : 이름, {point} : 적립포인트, {role} : 회원등급', 'mshop-point-ex' )
							)
						)
					)
				)
		...
		static function get_setting_rule_tab() {
			$target_search_rule_types = apply_filters( 'msps_target_search_rule_types', array( 'product', 'category', 'shipping-class' ) );
					...
					array(
						...
						'elements' => array(
							array(
								...
								"type"      => "Toggle",
								"default"   => "no",
								"desc"      => __( "<div class='desc2'>고객의 주문이 완료되면 일정 금액을 포인트로 적립해주는 기능을 사용합니다.</div>", 'mshop-point-ex' )
							),
							array(
								"id"        => "msps_deduct_point_for_partial_refunded",
								"title"     => __( "부분환불 시 포인트 차감", 'mshop-point-ex' ),
								'showIf'    => array( 'mshop_point_system_use_purchase_point_rule' => 'yes' ),
								"className" => "",
								"type"      => "Toggle",
								"default"   => "no",
								"desc"      => __( "<div class='desc2'>부분환불된 금액만큼 적립(예정) 포인트를 차감합니다.<br><span style='color: red'>(주의) 차감되는 포인트는 전체 주문금액 대비 부분환불된 금액의 비율만큼 차감되며, 상품별 정책 설정은 적용되지 않습니다.</span></div>", 'mshop-point-ex' )
							),
							array(
								"id"        => "msps_apply_filxed_point_by_multiplying_product_qty",
								"title"     => __( "상품 개수만큼 고정적립", 'mshop-point-ex' ),
								'showIf'    => array( 'mshop_point_system_use_purchase_point_rule' => 'yes' ),
								"className" => "",
								"type"      => "Toggle",
								"default"   => "no",
								"desc"      => __( "<div class='desc2'>고정적립 포인트를 구매한 상품 개수만큼 적립합니다. (고정적립 포인트 x 상품 개수)</div>", 'mshop-point-ex' )
							)
						)
					),
					array(
							...
							'left'        => array(
								...
								'elements'          => array(
									array(
										...
										"className"      => "fluid",
										'titleClassName' => 'five wide',
										"type"           => "Select",
										'default'        => 'product',
										'options'        => apply_filters( 'msps_rule_types', array(
									...
									array(
										...
										"showIf"      => array( "type" => implode( ',', $target_search_rule_types ) ),
										"className"   => "search fluid",
										'multiple'    => true,
										'search'      => true,
										'action'      => mshop_wpml_get_default_language_args() . 'action=' . msps_ajax_command( 'target_search&type=' ),
										"type"        => "SearchSelect",
										'options'     => array(),
									),
									array(
										...
										"tooltip"   => array(
											"title" => array(
												"content" => __( "규칙을 적용할 기간을 지정합니다.", 'mshop-point-ex' )
											)
										)
									),
									array(
									...
									array(
										...
										"tooltip"   => array(
											"title" => array(
												"content" => __( "다른 정책의 적용여부와 관계없이 항상 적용됩니다.", 'mshop-point-ex' )
											)
										)
									),
									array(
										"id"        => "exclude_sales",
										"type"      => "Toggle",
										"title"     => __( "할인상품제외", 'mshop-point-ex' ),
										"className" => "fluid",
										"default"   => "no",
										"tooltip"   => array(
											"title" => array(
												"content" => __( "할인상품은 포인트를 적립하지 않습니다.", 'mshop-point-ex' )
											)
										)
									)
								)
							...
							'price_rules' => array(
								...
								"elements"     => array(
									'left'  => array(
										'type'              => 'Section',
										"hideSectionHeader" => true,
										'elements'          => apply_filters( 'msps_price_rule_condition',
											array(
												array(
													"id"             => "amount",
													"type"           => "LabeledInput",
		...
		static function get_setting_user_tab() {
			return array(
				'type'     => 'Page',
				'title'    => __( '사용자 설정', 'mshop-point-ex' ),
				'class'    => '',
				'elements' => array(
					array(
						'type'     => 'Section',
						...
						'elements' => array(
							array(
								...
								"type"      => "Toggle",
								"default"   => "no",
								"desc"      => __( "<div class='desc2'>회원 가입시 사용자에게 포인트를 적립합니다.</div>", 'mshop-point-ex' )
							),
							array(
					...
					array(
						'type'     => 'Section',
						'title'    => __( '방문 포인트 적립 기능', 'mshop-point-ex' ),
						'elements' => array(
							array(
								"id"        => "msps_use_visit_point_rule",
								"title"     => __( "활성화", 'mshop-point-ex' ),
								"className" => "",
								"type"      => "Toggle",
								"default"   => "no",
								"desc"      => __( "<div class='desc2'>사용자가 사이트에 로그인 시 사용자에게 포인트를 적립합니다. (매주, 매달 설정은 추후 업데이트 될 예정입니다.)</div>", 'mshop-point-ex' )

							),
							array(
								'id'      => 'mshop_point_system_notice_at_visit',
								'showIf'  => array( 'msps_use_visit_point_rule' => 'yes' ),
								'title'   => __( '방문 포인트 적립 안내 문구', 'mshop-point-ex' ),
								'default' => __( '매일 방문 포인트 {point} 포인트가 적립되었습니다.', 'mshop-point-ex' ),
								'type'    => 'TextArea'
							),
							array(
								"id"        => "msps_visit_point_rule",
								"className" => "",
								'showIf'    => array( 'msps_use_visit_point_rule' => 'yes' ),
								"type"      => "SortableTable",
								"repeater"  => true,
								...
								"elements"  => array(
									array(
										"id"        => "name",
										"title"     => __( "회원등급", 'mshop-point-ex' ),
										"className" => "",
										"type"      => "Label"
									),
									array(
										"id"          => "day",
										"title"       => __( "매일", 'mshop-point-ex' ),
										"className"   => " fluid",
										"type"        => "LabeledInput",
										'inputType'   => 'number',
										"valueType"   => "unsigned int",
					...
					array(
						'type'     => 'Section',
						'title'    => __( '문자수신동의 사용자 추가 포인트 적립 기능', 'mshop-point-ex' ),
						'elements' => array(
							array(
								"id"        => "msps_use_sms_point_rule",
								"title"     => __( "활성화", 'mshop-point-ex' ),
								"className" => "",
								"type"      => "Toggle",
								"default"   => "no",
								"desc"      => __( "<div class='desc2'>문자수신에 동의한 사용자에 대해 구매금액의 대해 추가 포인트를 적립해줍니다.</div>", 'mshop-point-ex' )
							),
							array(
								"id"        => "msps_sms_point_rule",
								"className" => "",
								'showIf'    => array( 'msps_use_sms_point_rule' => 'yes' ),
								"type"      => "SortableTable",
								"repeater"  => true,
								"default"   => self::get_role_options( array( 'ratio' => '0', 'fixed' => '0' ) ),
								"elements"  => array(
									array(
										"id"        => "name",
										"title"     => __( "회원등급", 'mshop-point-ex' ),
									...
									array(
										...
										'inputType'   => 'number',
										"valueType"   => "unsigned float",
										"label"       => __( "%", 'mshop-point-ex' ),
										"default"     => "0",
										"placeholder" => "0"
									),
									array(
										"id"          => "fixed",
										"title"       => __( "고정포인트 적립", 'mshop-point-ex' ),
										"className"   => " six wide column fluid",
										"type"        => "LabeledInput",
										'inputType'   => 'number',
										"valueType"   => "unsigned float",
							...
							'left'  => array(
								...
								'elements'          => array(
									...
									array(
										...
										"className"   => "search fluid",
										'multiple'    => true,
										'action'      => mshop_wpml_get_default_language_args() . 'action=' . msps_ajax_command( 'target_search&type=taxonomy&taxonomy=' ),
										"type"        => "SearchSelect",
										'options'     => array(),
										"tooltip"     => array(
											"title" => array(
									...
									array(
										"id"          => "post_length",
										"title"       => __( "게시글 길이 조건", 'mshop-point-ex' ),
										"className"   => " fluid",
										"type"        => "LabeledInput",
										'inputType'   => 'number',
										"valueType"   => "unsigned int",
										"label"       => __( "글자 이상", 'mshop-point-ex' ),
										"default"     => "",
										"placeholder" => "0"
									),
									array(
										"id"          => "comment_length",
										"title"       => __( "댓글 길이 조건", 'mshop-point-ex' ),
										"className"   => " fluid",
										"type"        => "LabeledInput",
										'inputType'   => 'number',
										"valueType"   => "unsigned int",
		...
		public static function output() {
			...
			wp_localize_script( 'mshop-setting-manager', 'mshop_setting_manager', array(
				...
				'action'   => msps_ajax_command( 'update_msps_policy_settings' ),
				'settings' => $settings,
			) );
