...
<tr>
    <td class="point-title" style="text-align:left">
        ...
        <p style='font-size: 0.9em; color: gray;'><?php echo sprintf( __( '보유 포인트 : %s', 'mshop-point-ex' ), number_format( floatval( $user_point ), wc_get_price_decimals() ) ); ?></p>
        <p style='font-size: 0.9em; color: gray;'><?php echo sprintf( __( '결제 가용 포인트 : %s', 'mshop-point-ex' ), $user_point > $max_useable_point ? number_format( floatval( $max_useable_point ), wc_get_price_decimals() ) : number_format( floatval( $user_point ), wc_get_price_decimals() ) ); ?></p>
		<?php
