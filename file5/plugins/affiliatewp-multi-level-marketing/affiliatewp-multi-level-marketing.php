...
class AFFWP_MLM_License {
	...
	static function init() {
		...
		if ( ! class_exists( 'AFFWP_MLM_License_Menu' ) ) {
			...
			AFFWP_MLM_License_Menu::instance( __FILE__, 'AffiliateWP MLM', '1.1.7', 'plugin', 'http://propluginmarketplace.com' );
		}
	}
...
if ( ! class_exists( 'AffiliateWP_Multi_Level_Marketing' ) ) {
	...
	final class AffiliateWP_Multi_Level_Marketing {
		...
		private function includes() {
			...
			require_once AFFWP_MLM_PLUGIN_DIR . 'includes/email-functions.php';
			require_once AFFWP_MLM_PLUGIN_DIR . 'includes/scripts.php';
			require_once AFFWP_MLM_PLUGIN_DIR . 'includes/actions.php';
			require_once AFFWP_MLM_PLUGIN_DIR . 'includes/email-actions.php';
		...
		public function get_theme_template_paths( $file_paths ) {
			$file_paths[] = plugin_dir_path( __FILE__ ) . '/templates';
		...
		public function display_indirect_referrals_in_tab( $affiliate_id ) {
			...
			if ( empty( $integrations ) ) return; // Indirect referrals are disabled
			
			if ( isset( $_GET['tab'] ) && 'referrals' != $_GET['tab'] ) return;
			
			// Don't show indirect referrals if Sub Affiliates aren't allowed
			if ( ! affwp_mlm_is_parent_affiliate( $affiliate_id ) && ! affwp_mlm_sub_affiliate_allowed( $affiliate_id ) ) return;		
