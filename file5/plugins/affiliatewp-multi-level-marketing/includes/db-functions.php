...
function affwp_mlm_get_parent_affiliate( $affiliate_id = 0 ) {
	...
	$parent_affiliate_id = $affiliate_connections ? $affiliate_connections->affiliate_parent_id : 0;
...
function affwp_mlm_get_direct_affiliate( $affiliate_id = 0 ) {
	...
	// Get the affiliate's direct affiliate
	$affiliate_connections = affwp_mlm_get_affiliate_connections( $affiliate_id );
	$direct_affiliate_id = $affiliate_connections ? $affiliate_connections->direct_affiliate_id : 0;
...
function affwp_mlm_get_complete_cycles( $affiliate_id = 0 ) {
	...
	$cycles = is_array( $complete_cycles ) ? end( $complete_cycles ) : 0;
 ...
 * @return array
 */
function affwp_mlm_get_spillover_affiliates( $affiliate_id = 0, $relation = 'direct' ) {
	
	$spillover_ids = array();
	
	// The given affiliate is the direct referrer, but not the parent (The sender)
	if ( $relation == 'direct' ) {

		$sub_affiliates = affwp_mlm_get_direct_sub_affiliates( absint( $affiliate_id ) );
		...
		foreach ( $sub_ids as $sub_id ) {
			...
			// Check for a different parent affiliate
	...
	// The given affiliate is the parent, but not the direct affiliate (The receiver)
	if ( $relation == 'parent' ) {

		$sub_affiliates = affwp_mlm_get_sub_affiliates( absint( $affiliate_id ) );
		$sub_ids = wp_list_pluck( $sub_affiliates, 'affiliate_id' );

		foreach ( $sub_ids as $sub_id ) {
			
			$direct_aff_id = affwp_mlm_get_direct_affiliate( $sub_id );

			// Check for a different direct affiliate
			if ( $direct_aff_id != $affiliate_id ) {
 ...
 * @since 1.0
 */
function affwp_mlm_find_open_affiliate( $affiliate_id = 0, $max_depth = 0 ) {

	if ( empty( $affiliate_id ) ) return;

	$downline = affwp_mlm_get_downline( $affiliate_id, $max_depth );
	$level_count = 0;
